...
const getVoiceTokenFetch = () =>
  fetch("/chat/voice-token")
    .then((r) => r.json())
    .catch(() => ({ error: "Failed to get voice token" }));
...
export function ChatInput({
  ...
  const handleSubmit = (e) => {
    ...
    if (
      disabled ||
      (!input.trim() && !partialText.trim() && files.length === 0) ||
      isStreaming
    )
      return;
      ...
      >
        ...
        <textarea
          ...
          value={
            input +
            (partialText
              ? (input && !input.endsWith(" ") ? " " : "") + partialText
              : "")
          }
          onChange={(e) => {
